...
${ ({bigPhoto, editPhoto, fadeIn, imgPosition}) => ( bigPhoto && !editPhoto) && css`
    & { 
        background-image: url(${ `${imgFolder}/${bigPhoto?.id}b.jpg?${imgPosition.reload}` });
        ${ fadeIn && css`
            opacity: 0;
...
${ ({bigPhoto, editPhoto, fadeIn, imgPosition}) => editPhoto && css`
    &::after {
        background-image: url(${ (editPhoto?.url as string ) || `${imgFolder}/${bigPhoto?.id}b.jpg?${imgPosition.reload}` });
